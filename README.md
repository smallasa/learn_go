...
问题解析：  
在同一时刻，Go语言的运行时系统（简称运行时系统）只会执行对同一个通道的任意各发送操作中的某一个。
...
又比如，接收操作在准备好元素值副本之后，一定会删除掉通道中的原值，觉不会出现通道中仍有残留的情况。
...
对于非缓冲通道，情况要简单一些。无论是发送操作还是接收操作，一开始执行就会被阻塞，直到配对的操作也开始执行。
...
首先是在初始化通道时设定其容量的定义，这有时会让通道拥有不同的行为模式。对通道的发送操作和接收操作都有那些基本特性，也是我们必须清楚的。
...
解答，通道的长度，表示channel的长度。当channel处于阻塞状态时，容纳最多的同类型的长度。

2.元素值在经过通道传递时会被复制，那么这个复制是浅表复制还是深层复制？ 
解答，深层拷贝。因为通道类型使用的是引用类型。
